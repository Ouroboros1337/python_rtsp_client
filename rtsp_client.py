...
def _rtsp_stream(rtsp_link:str, use_udp:bool = True) -> Iterator[str]:
...
def _background_capture(rtsp_link:str, request_list: list[int], feedback_list: list[ndarray], use_udp:bool = True):

    stream = _rtsp_stream(rtsp_link,use_udp)
    while True:
...
def _get_image(request_list: list[int], feedback_list: list[ndarray]) -> ndarray:
    ...
    assert len(request_list) <= 1, f'You are calling the fetch function multiple times, this wont work'
...
def run_non_blocking(rtsp_link:str, use_udp:bool = True) -> Callable:
    ...
    capture = Thread(target=_background_capture,
                     args=(rtsp_link, request_list, feedback_list, use_udp))
    capture.start()
...
def run_blocking(rtsp_link:str, use_udp:bool = True) -> Callable:
